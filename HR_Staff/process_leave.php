...
if (isset($_GET['id']) && isset($_GET['status'])) {
   ...
   if ($new_status === 'Approved' || $new_status === 'Rejected') {
        ...
        // Changed variable name to $remarks
        $remarks = isset($_GET['reason']) ? mysqli_real_escape_string($mysql, urldecode($_GET['reason'])) : '';
        
        if ($new_status === 'Rejected') {
            // Updated column name to 'remarks'
            $updateQuery = "UPDATE leave_requests SET status = '$new_status', remarks = '$remarks' WHERE leave_id = $leave_id";
        } else {
