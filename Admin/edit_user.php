...
$display_middle = !empty($userProfile['middle_name']) ? $userProfile['middle_name'] : '';
$display_last  = !empty($userProfile['last_name']) ? $userProfile['last_name'] : $userProfile['u_last'];
$display_suffix = !empty($userProfile['suffix']) ? $userProfile['suffix'] : '';
...
$current_shift_start = !empty($userProfile['shift_start']) ? $userProfile['shift_start'] : '08:00:00';
$current_shift_end = !empty($userProfile['shift_end']) ? $userProfile['shift_end'] : '17:00:00';
    ...
    if (!$has_error) {
        ...
        try {
            ...
            if ($checkEmp->get_result()->num_rows > 0) {
                $updateEmp = $mysql->prepare("UPDATE employee_details SET first_name=?, middle_name=?, last_name=?, suffix=?, email=?, gender=?, birth_date=?, mobile_number=?, address=?, position=?, role=?, profile_image=?, shift_start=?, shift_end=? WHERE user_id=?");
                $updateEmp->bind_param("ssssssssssssssi", $first_name, $middle_name, $last_name, $suffix, $email, $gender, $birth_date, $mobile_number, $address, $position, $role, $profile_image_path, $shift_start, $shift_end, $id);
                $updateEmp->execute();
            } else {
                ...
                $insertEmp->bind_param("issssssssssssss", $id, $first_name, $middle_name, $last_name, $suffix, $email, $gender, $birth_date, $mobile_number, $address, $position, $role, $profile_image_path, $shift_start, $shift_end);
                $insertEmp->execute();
            }
                ...
                <div class="card-body bg-light">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-2 form-group">
                        ...
                        <input type="text" name="suffix" class="form-control shadow-sm" value="<?php echo htmlspecialchars($display_suffix); ?>" placeholder="Jr, Sr, III">
                    </div>
                  </div>
                  ...
                  <div class="row">
                    <div class="col-md-6 form-group">
                        <label class="text-dark"><i class="fas fa-clock mr-1 text-primary"></i> Assigned Shift Schedule <span class="text-danger">*</span></label>
                    ...
                    <div class="col-md-3 form-group">
                        ...
                        <select name="role" class="form-control shadow-sm" required>
                            <option value="Employee" <?php echo ($display_role == 'Employee') ? 'selected' : ''; ?>>Employee</option>
                            <option value="HR Staff" <?php echo ($display_role == 'HR Staff') ? 'selected' : ''; ?>>HR Staff</option>
                            <option value="Admin" <?php echo ($display_role == 'Admin') ? 'selected' : ''; ?>>System Admin</option>
                        </select>
                    </div>
                    <div class="col-md-3 form-group">
